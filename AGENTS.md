...
Rewriting published history, including timestamp-only corrections, can break CI/CD even when every file tree remains unchanged. Before such a rewrite, read the versioning guide and inspect the affected release records and push-validation range. Treat this as exceptional metadata repair: preserve release trees and `Harness-Release-Run` identities, update each affected `Harness-Source` trailer to its rewritten parent, and validate every affected release before pushing.

The September 2026 incident exposed two failures. [Run 35401568223](https://github.com/kylesaburao/harness-plugin/actions/runs/35401568223) rejected a stale source trailer with `INVALID_RELEASE_ANCHOR`. After repair, [run 35402183205](https://github.com/kylesaburao/harness-plugin/actions/runs/35402183205) failed with `INVALID_REVISION` because the force-push event’s old `before` commit was absent from the runner. Keeping that object available does not resolve the ancestry problem: the same range fails with `NON_ANCESTOR_SOURCE_RANGE`. Empty commit `54b97f8eb57404b843def05b5bfef2247cfd94f7` subsequently supplied a normal push range and publication succeeded.

For an authorized repair, retain a local backup, wait for publication work to finish, and use an explicit expected-SHA lease. Keep Actions enabled: push the validated rewrite with a new ordinary `[skip ci]` commit, confirm the remote tip, then separately push a normal follow-up commit without a skip instruction. Verify that follow-up CI succeeds. Rerunning the original failed event retains its obsolete baseline.
