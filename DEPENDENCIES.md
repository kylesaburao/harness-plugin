...
| --- | --- |
| `harness-advisor` | Node.js **22.0.0 or newer**, no npm packages. Sparse user routing at `~/.harness-plugin/harness-advisor/config.json`, managed by `advisor-config.js`. Codex requires ordinary fresh-agent dispatch with explicit model/effort and the canonical Advisor contract. Additional child permission controls are used when available, with instructions distinguished from enforced permissions. Claude fallback requires a local Claude Code CLI accepting the restrictive invocation in `claude-advisor.js`. The contract is supplied through `--system-prompt` and saved settings are excluded with `--setting-sources ''`. Neither host requires a persistent role. Preflight checks readable nonempty prompt/contract and `claude --version`, not authentication, model access, or runtime enforcement. Authentication helpers in excluded settings require separate qualification. Claude is installed but not logged in, so live qualification is skipped. Each host needs access to the resolved family/effort. Native Claude suppresses this Skill entirely. |
| `install-harness-plugin-capabilities` | Instruction-only, no dedicated runtime, package, or host CLI requirement. Uses ordinary agent filesystem tools to install Advisor activation, Final plans guidance, and Skill discovery on the effective user-level instruction surface for Codex or Claude Code. Copies the bundled `final-plan-context.md` into absolute `CODEX_HOME` or `~/.codex`, or absolute `CLAUDE_CONFIG_DIR` or `~/.claude`. Preserves legacy role files, host settings, and Advisor routing. |
