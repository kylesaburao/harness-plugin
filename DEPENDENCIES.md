...
| `install-harness-plugin-capabilities` | Node.js **22.0.0 or newer**, no npm packages or host CLIs required. Writes only a managed activation block in `AGENTS.md` under absolute `CODEX_HOME` or `~/.codex`, or `CLAUDE.md` under absolute `CLAUDE_CONFIG_DIR` or `~/.claude`. Preserves legacy role files, host settings, and Advisor routing. Tests use temporary homes and simulated CLIs, with no live inference required. |
