...
| `install-harness-plugin-capabilities` | Instruction-only, no dedicated runtime, package, or host CLI requirement. Uses ordinary agent filesystem tools to install Advisor activation and Final plans guidance on the effective user-level instruction surface for Codex or Claude Code. Copies the bundled `final-plan-context.md` into absolute `CODEX_HOME` or `~/.codex`, or absolute `CLAUDE_CONFIG_DIR` or `~/.claude`. Preserves legacy role files, host settings, and Advisor routing. |
