...
```sh
cp "<SKILL_DIR>/references/backup-config.json" backup-config.local.json
```

Validate it without backing anything up:

```sh
node "<SKILL_DIR>/scripts/backup.js" --preflight --json ./backup-config.local.json
```

Run the backup with the local configuration:

```sh
node "<SKILL_DIR>/scripts/backup.js" ./backup-config.local.json
```
