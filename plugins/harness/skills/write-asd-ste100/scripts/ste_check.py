...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    try:
        terms = load_terms(args.terms) if "project" in args.layers else {}
    except ValueError as error:
        report_invocation_error(
            InvocationError("terms_invalid", str(error), remedy=f"correct the terminology file {args.terms} or omit --terms"),
            args.json,
        )
        return 2
    try:
