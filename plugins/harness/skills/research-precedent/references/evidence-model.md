...
Use the tier selected in `SKILL.md`. This is the full structure. The brief tier compresses Strongest precedent, Contrary precedent, External state of the art, Differences from the current situation, and Confidence and unknowns, per the definition in `SKILL.md`. The verdict line is the conclusion plus one coordinate.
...
**External state of the art.** What the upstream project, the standard, or common practice in this ecosystem currently does about this problem class, and how the proposed approach sits relative to it. Cite primary sources. Where practice has converged, say so and say on what. Where it has not, say what the live disagreement is about, because a live disagreement is more informative than a fabricated majority. This section carries real weight in every report, per the research budget in `SKILL.md`, not only in a thin repository.
...
**Confidence and unknowns.** Sources that could not be reached, uncertain outcomes, documentation suspected of being stale, unresolved contradictions, assumptions made, and the places where more research could still change the conclusion. Name the external and organizational source classes that were unreachable explicitly, not just the ones that were searched and came up empty. "The organization's issue tracker was not reachable" and "the organization's issue tracker was searched and had nothing" are different claims that support different amounts of confidence, and collapsing them is the same failure the invariant against reporting "no precedent exists" already guards against.
