...
test('explicit empty values do not fall back and both modes require MAC and IP', () => {
  const env = { MAC_ADDRESS: MAC, IP_ADDRESS: 'desktop.local', TIMEOUT: '30' };
  for (const flag of ['--mac=', '--ip=', '--timeout=']) {
    assert.throws(() => validateConfiguration(parseArguments([flag, '--no-wait'], env)));
  }
  for (const args of [[], ['--no-wait']]) {
    assert.throws(() => validateConfiguration(parseArguments([...args, '--mac', MAC])));
    assert.throws(() => validateConfiguration(parseArguments([...args, '--ip', 'desktop.local'])));
  }
  assert.throws(() => validateConfiguration(parseArguments([], { ...env, TIMEOUT: '' })));
});
test('seconds and milliseconds must both be safe integers', () => {
  for (const timeout of ['9007199254740992', '9007199254741', '1'.repeat(400), '1.5', '-1', '01']) {
    assert.equal(startupCode(() => validateConfiguration(parseArguments(
      ['--mac', MAC, '--ip', 'desktop.local', `--timeout=${timeout}`]))), 'config_invalid');
  }
  assert.equal(validateConfiguration(parseArguments(['-m', MAC, '-i', 'desktop.local', '-t', '9007199254740'])).timeoutSeconds, 9007199254740);
});
