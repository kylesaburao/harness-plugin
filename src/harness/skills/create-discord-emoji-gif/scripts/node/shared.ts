...
export interface GifCommonConfig { maxBytes: number; gifSize: number; minFps: number; maxFps: number; jobs: number; keepWork: boolean }
export interface GifskiConfig extends GifCommonConfig { minQuality: number; maxQuality: number }
export type GifsicleConfig = GifCommonConfig;
export type GifConfiguration<B extends GifBackend> = B extends 'gifski' ? GifskiConfig : GifsicleConfig;
export interface CandidateResult { fps: number; bytes: number; score: string; path: string; digest: string }
export interface GifskiParameters { quality: number; motionQuality: number; lossyQuality: number }
export interface GifsicleParameters { colors: number; dither: number }
export type GifskiCandidate = CandidateResult & GifskiParameters;
export type GifsicleCandidate = CandidateResult & GifsicleParameters;
export interface ConverterState<B extends GifBackend> {
  input: string; inputPath: string; output: string; outputDir: string; workDir: string; outputTemp: string;
  json: boolean; scriptName: string; config: GifConfiguration<B>; commands: ReadyCommands<B>; manager: ProcessManager;
}
export interface ScoringState {
  manager: ProcessManager; workDir: string; referenceFrames: number;
  commands: Pick<ReadyCommands, 'ffmpeg' | 'ffprobe'>; config: Pick<GifCommonConfig, 'keepWork'>;
}
export type CandidateScorer = (candidate: string, task: string, candidateFps: number, digest: string) => Promise<string>;
export interface GifWarning { code: string; condition: string; recommendation: string }
export type GifResult = ReturnType<typeof resultPayload> & { cleanupFailures?: CleanupFailure[] };
type ResultInput = { script: string; input: string; output: string; config: GifCommonConfig; verified: VerifiedGif } & (
  | { backend: 'gifski'; winner: GifskiCandidate }
  | { backend: 'gifsicle'; winner: GifsicleCandidate }
);

const MAX_EXACT_INTEGER = 9007199254740991n;

function parseArguments(argv: string[], basename = 'mov-to-gif.js') {
  const positional = [];
  let preflight = false;
  let json = false;
  let help = false;
...
function readConfiguration<B extends GifBackend>(env: NodeJS.ProcessEnv, backend: B): GifConfiguration<B>;
function readConfiguration(env: NodeJS.ProcessEnv, backend: GifBackend): GifCommonConfig | GifskiConfig {
  const common = {
  ...
  if (common.minFps > common.maxFps) throw new StartupError('config_invalid', `MIN_FPS (${common.minFps}) must not exceed MAX_FPS (${common.maxFps})`, 'set MIN_FPS at or below MAX_FPS, or unset both to take the defaults');
  if (backend === 'gifski') {
    const config = Object.assign(common, { minQuality: positive(env, 'MIN_QUALITY', 1), maxQuality: positive(env, 'MAX_QUALITY', 100) });
    for (const [name, value] of [['MIN_QUALITY', config.minQuality], ['MAX_QUALITY', config.maxQuality]] as const) {
...
async function inspectInput(manager: ProcessManager, commands: Pick<ReadyCommands, 'ffmpeg' | 'ffprobe'>, input: string): Promise<GifWarning[]> {
...
function validateOutput(input: string, requested: string | undefined, size: number) {
...
function parseVmafScore(text: string) {
  let report: unknown;
...
async function referenceFrameCount(state: Pick<ScoringState, 'manager' | 'commands' | 'workDir'>) {
...
async function scoreCandidate(manager: ProcessManager, commands: Pick<ReadyCommands, 'ffmpeg' | 'ffprobe'>, workDir: string, candidate: string, task: string, referenceFrames: number, candidateFps: number, keepWork = false) {
  ...
  try {
    ...
    let report;
    try { report = parseVmafScore(text); } catch (error) { if (error === null || typeof error !== 'object') throw error; throw Object.assign(error, childDetails(task, result)); }
...
function createCandidateScorer(state: ScoringState): CandidateScorer {
  const { manager, workDir, referenceFrames } = state;
  const commands = { ...state.commands };
  const keepWork = state.config.keepWork;
  const scores = new Map<string, Promise<string>>();
...
function cleanupArtifacts({ workDir, outputTemp, config }: Pick<ConverterState<GifBackend>, 'workDir' | 'outputTemp' | 'config'>): CleanupFailure[] {
  const failures = [];
  for (const target of [outputTemp, config.keepWork ? null : workDir].filter((value): value is string => Boolean(value))) {
    try { fs.rmSync(target, { recursive: true, force: true }); }
    catch (error) { failures.push({ path: target, code: fieldsOf(error).code, condition: fieldsOf(error).message }); }
  }
  if (workDir && fs.existsSync(workDir) && config.keepWork) process.stderr.write(`Kept work directory: ${workDir}\n`);
  return failures;
}

function emitWarnings(warnings: GifWarning[], json: boolean) {
...
function emitPreflightReady(state: ToolchainPreflight, warnings: GifWarning[], json: boolean) {
...
function formatParameters(parameters: GifskiParameters | GifsicleParameters) {
...
function resultPayload({ script, backend, input, output, config, winner, verified }: ResultInput) {
...
function emitResult(payload: GifResult, json = false) {
...
function preflightError(state: ToolchainPreflight) {
...
function usage(backend: GifBackend, basename: string) {
