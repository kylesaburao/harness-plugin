import { resolveCommand } from '../../../../shared/node/resolve-command.js';
import { StartupError } from './errors.js';
import type { PreflightFailure } from './errors.js';
import type { ProcessManager } from './process-manager.js';
export type GifBackend = 'gifski' | 'gifsicle';
type ToolName = 'ffmpeg' | 'ffprobe' | GifBackend;
export type ReadyCommands<B extends GifBackend = GifBackend> = { ffmpeg: string; ffprobe: string } &
  (B extends 'gifski' ? { gifski: string } : { gifsicle: string });
export interface ToolchainPreflight {
  policy: ReturnType<typeof platformPolicy>;
  commands: Partial<Record<ToolName, string>>;
  failures: PreflightFailure[];
}
export function platformPolicy(platform: string, backend: GifBackend) {
  ...
  const commandRemedy = (name: ToolName) => {
    if (mac) {
...
async function probe(manager: ProcessManager, task: string, command: string, args: string[]) {
  const result = await manager.runOwned(task, command, args, { stdout: 'capture', stderr: 'capture' });
  return { ...result, output: result.stdout + result.stderr };
}

function listingContains(listing: string, wanted: string) {
...
function optionListingContains(listing: string, wanted: string) {
  return listing.split(/[\s,]+/).some(token => token === wanted || token.startsWith(`${wanted}=`) || token.startsWith(`${wanted}[`));
}

function toolRemedy(platform: string, tool: ToolName, action: string) {
...
async function checkFfprobePreflight(manager: ProcessManager, state: Pick<ToolchainPreflight, 'commands' | 'failures'>, platform: string) {
...
async function checkCommonPreflight(manager: ProcessManager, backend: GifBackend, platform: string = process.platform, env: NodeJS.ProcessEnv = process.env): Promise<ToolchainPreflight> {
  const policy = platformPolicy(platform, backend);
  const names: ToolName[] = ['ffmpeg', 'ffprobe', backend];
  const commands: ToolchainPreflight['commands'] = {};
...
export async function checkGifskiPreflight(manager: ProcessManager, platform: string, env: NodeJS.ProcessEnv) {
...
export async function checkGifsiclePreflight(manager: ProcessManager, platform: string, env: NodeJS.ProcessEnv) {
...
export function requireReadyCommands<B extends GifBackend>(state: ToolchainPreflight, backend: B): ReadyCommands<B>;
export function requireReadyCommands(state: ToolchainPreflight, backend: GifBackend): ReadyCommands {
  const { ffmpeg, ffprobe } = state.commands;
  const encoder = state.commands[backend];
  if (!ffmpeg || !ffprobe || !encoder) throw new StartupError('command_missing', 'required commands are not ready', state.policy.installRemedy);
