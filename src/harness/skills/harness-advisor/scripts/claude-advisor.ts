...
{
    return EFFORTS.some(item => item === value);
}
function details(error: unknown)
{
    ...
    return {
        ...
        remedy: typeof value.remedy === 'string' ? value.remedy : undefined,
    };
}
...
{
    throw Object.assign(new Error(condition), { code, condition, remedy });
}
export function parseArguments(argv: string[]): ClaudeAdvisorOptions | HelpOptions
{
    if (argv.some(arg => arg === '--workspace' || arg.startsWith('--workspace=')))
    {
        fail('usage_error', '--workspace is no longer supported; Harness Advisor uses executor-supplied evidence only.',
            'Remove --workspace and its value; include the material source excerpts and results in --prompt.');
    }
...
export function invocation(options: Pick<ClaudeAdvisorOptions, 'model' | 'reasoning-effort'>, contract: string, env: NodeJS.ProcessEnv = process.env): ClaudeInvocation
{
    ...
    delete childEnv.CLAUDECODE; // Separate print session; never share the parent transcript.
    return {
        command: 'claude',
        args: ['-p', '--system-prompt', contract, '--setting-sources', '', '--model', options.model,
...
function main(argv: string[])
{
    ...
    try
    {
        ...
        {
            // Report the existing reinstall remedy below.
        }
        if (!contract?.trim())
        {
            fail('advisor_contract_unavailable', `Advisor contract is missing, unreadable, or empty at ${contractFile}`,
        ...
        else
        {
            ...
            {
                fail('advisor_execution_failed', `Claude Advisor ${options.model} failed: ${result.error?.message || result.stderr || result.stdout}`);
            }
