...
function parseArguments(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (!argv[i].startsWith('--') || !['native-absent', 'model', 'reasoning-effort', 'prompt', 'json', 'preflight', 'help'].includes(key) || Object.hasOwn(options, key))
...
function main(argv) {
    ...
    try {
        ...
        const common = { model: options.model, reasoning_effort: options['reasoning-effort'], mechanism: 'claude-cli', context_mode: 'fresh', tools: [] };
        let report;
