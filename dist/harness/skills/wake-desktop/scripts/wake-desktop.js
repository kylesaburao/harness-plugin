...
exports.nodeVersionAtLeast = exports.isValidIpv4 = exports.StartupError = exports.EXIT = void 0;
exports.buildMagicPacket = buildMagicPacket;
exports.parseArguments = parseArguments;
exports.validateConfiguration = validateConfiguration;
exports.waitForHost = waitForHost;
...
Object.defineProperty(exports, "StartupError", { enumerable: true, get: function () { return target_config_js_1.StartupError; } });
Object.defineProperty(exports, "isValidIpv4", { enumerable: true, get: function () { return target_config_js_1.isValidIpv4; } });
Object.defineProperty(exports, "nodeVersionAtLeast", { enumerable: true, get: function () { return target_config_js_1.nodeVersionAtLeast; } });
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (takesValue.has(argument)) {
            ...
            if (value === undefined || value.startsWith('-')) {
                throw new target_config_js_1.StartupError('usage_error', `${argument} requires a value`, 'run with --help to see the accepted options');
            }
        ...
        switch (argument) {
            ...
            case '--help':
                ...
                break;
            default:
                throw new target_config_js_1.StartupError('usage_error', `unknown argument: ${argument}`, 'run with --help to see the accepted options');
        }
    }
    // Explicit flags win over environment fallbacks.
    if (options.target === null && options.mac === null && env.MAC_ADDRESS !== undefined)
        options.mac = env.MAC_ADDRESS;
    if (options.target === null && options.ip === null && env.IP_ADDRESS !== undefined)
        options.ip = env.IP_ADDRESS;
    if (options.timeout === null && env.TIMEOUT !== undefined)
...
function validateConfiguration(options) {
    const missing = [];
    if (!options.mac)
        missing.push('MAC address (--mac or MAC_ADDRESS)');
    if (!options.ip)
        missing.push('target host (--ip or IP_ADDRESS)');
    if (missing.length > 0) {
        throw new target_config_js_1.StartupError('config_missing', `no ${missing.join(' and no ')}`, 'pass --mac a1:b2:c3:d4:e5:f6 --ip 192.168.1.50, or set MAC_ADDRESS and IP_ADDRESS');
    }
    if (!(0, target_config_js_1.isValidMac)(options.mac)) {
        throw new target_config_js_1.StartupError('config_invalid', `MAC address is not six hex pairs separated by ':' or '-': ${options.mac}`, 'pass a MAC address such as a1:b2:c3:d4:e5:f6');
    }
    if (!(0, target_config_js_1.isValidHost)(options.ip)) {
        throw new target_config_js_1.StartupError('config_invalid', `target host is neither an IPv4 address nor a hostname: ${options.ip}`, 'pass an address such as 192.168.1.50, or a resolvable hostname');
    }
    ...
    if (!/^[1-9][0-9]*$/.test(rawTimeout) ||
        ...
        throw new target_config_js_1.StartupError('config_invalid', `timeout must be positive whole seconds with safe integer seconds and milliseconds, got '${rawTimeout}'`, 'pass --timeout 120, or unset TIMEOUT to take the default');
    }
    return {
        mac: options.mac,
        ip: options.ip,
...
function buildMagicPacket(mac) {
    ...
    if (bytes.length !== 6) {
        throw new target_config_js_1.StartupError('config_invalid', `MAC address did not decode to six bytes: ${mac}`, 'pass a MAC address such as a1:b2:c3:d4:e5:f6');
    }
...
async function checkEnvironment(options) {
    ...
    if (!['darwin', 'linux'].includes(os.platform())) {
        throw new target_config_js_1.StartupError('platform_unsupported', `unsupported platform: ${os.platform()}`, 'run on macOS or Linux on the target LAN');
    }
    if (options.wait) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const finish = (error) => {
            ...
            try {
                ...
                if ((0, target_config_js_1.errorDetails)(closeError).code !== 'ERR_SOCKET_DGRAM_NOT_RUNNING')
                    error = closeError;
            }
            if (error)
                reject(new target_config_js_1.StartupError('broadcast_unavailable', `UDP broadcast failed: ${(0, target_config_js_1.errorDetails)(error).message}`, 'run on the target LAN with UDP broadcast permitted'));
            else
                resolve();
        };
        try {
...
async function waitForHost(config) {
    ...
    throw new target_config_js_1.StartupError('host_unreachable', `${config.ip} did not answer within ${config.timeoutSeconds} seconds of the magic packet`, 'check target Wake-on-LAN firmware and network settings, the broadcast domain, and ICMP access. Stop without retrying');
}
...
async function main(argv, env) {
    ...
    try {
        ...
        if (options.target !== null) {
            const saved = (0, target_config_js_1.loadTarget)(options.target);
            if (options.mac === null)
                options.mac = saved.mac;
            if (options.ip === null)
                options.ip = saved.ip;
        }
