...
exports.assertDirectoryUnchanged = assertDirectoryUnchanged;
...
async function validateOutputDirectory(configuredPath, source) {
    ...
    const output = await validateDirectory(configuredPath, 'outputDirectory', fs.constants.R_OK | fs.constants.W_OK | fs.constants.X_OK);
...
function backupFilename(sourceDirectory, now = new Date()) {
    ...
    if (Buffer.byteLength(candidate) <= MAX_FILENAME_BYTES)
        return candidate;
...
async function pathKind(destination, label) {
    try {
        ...
        if (details.isDirectory())
            throw new Error(`${label} exists but is a directory: ${destination}`);
        return true;
    }
    catch (error) {
        if (errorDetails(error).code === 'ENOENT')
