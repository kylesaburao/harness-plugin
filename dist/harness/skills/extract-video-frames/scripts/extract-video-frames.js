...
exports.transformFromMatrix = exports.selectVideoStream = exports.parseTime = exports.identity = exports.formatTime = exports.displayRotation = exports.classifyStream = exports.boundedTail = exports.analyzeFrameSpool = exports.ProcessManager = exports.pixelProperties = exports.descriptorMap = void 0;
exports.cleanupPaths = cleanupPaths;
exports.emitError = emitError;
exports.assertSourceUnchanged = assertSourceUnchanged;
exports.codecArguments = codecArguments;
exports.colorConversionFilter = colorConversionFilter;
exports.convertHdrFrames = convertHdrFrames;
exports.decodeProbeArguments = decodeProbeArguments;
exports.derivePaths = derivePaths;
exports.ffmpegArguments = ffmpegArguments;
exports.inspectInput = inspectInput;
exports.parseArguments = parseArguments;
exports.prepare = prepare;
exports.publishDirectoryNoReplace = publishDirectoryNoReplace;
exports.representativeDecodePreflight = representativeDecodePreflight;
exports.resultPayload = resultPayload;
exports.structuralChecks = structuralChecks;
const fs = require("node:fs");
const os = require("node:os");
const path = require("node:path");
const commandResolution = require("../../../shared/node/resolve-command.js");
...
function parseArguments(argv, basename = 'extract-video-frames.js') {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (positionalOnly) {
            if (options.input !== null)
                throw new DraftError('usage_error', 'more than one input path was supplied', `run ${basename} --help`);
        ...
        if (flag === '--start' || flag === '--end') {
            ...
            if (!value || value.startsWith('--'))
                throw new DraftError('usage_error', `${flag} requires a time value`, `run ${basename} --help`);
            options[flag === '--start' ? 'start' : 'end'] = parseTime(value, flag);
            continue;
        }
        if (argument.startsWith('-'))
            throw new DraftError('usage_error', `unknown option: ${argument}`, `run ${basename} --help`);
        if (options.input !== null)
            throw new DraftError('usage_error', 'more than one input path was supplied', `run ${basename} --help`);
        options.input = argument;
    }
    if (options.help)
        return options;
    if (!options.input && !options.preflight)
        throw new DraftError('usage_error', 'INPUT_VIDEO is required', `run ${basename} INPUT_VIDEO`);
    if (!options.input && (options.start !== null || options.end !== null))
        throw new DraftError('usage_error', 'window flags require INPUT_VIDEO', `run ${basename} --preflight without window flags, or add INPUT_VIDEO`);
    if (options.start !== null && options.end !== null && options.start > options.end)
        throw new DraftError('window_invalid', '--start must not be later than --end', 'swap the bounds or pass an end time at or after the start time');
    return options;
}
...
function validateInputAndOutput(input) {
    ...
    if (!source.isFile())
        throw new DraftError('input_unusable', `input is not a regular file: ${paths.supplied}`, 'pass the path of an existing video file');
    try {
        fs.accessSync(paths.supplied, fs.constants.R_OK);
    }
    catch {
        throw new DraftError('input_unusable', `input is not readable: ${paths.supplied}`, 'grant read access or copy the video to a readable location');
    }
    if (pathExists(paths.output))
        throw new DraftError('output_collision', `output already exists: ${paths.output}`, 'move or remove the existing path, then run the same command again');
    try {
        fs.accessSync(path.dirname(paths.output), fs.constants.W_OK);
    }
    catch {
        throw new DraftError('output_unusable', `input directory is not writable: ${path.dirname(paths.output)}`, 'grant write access or place the input in a writable directory');
    }
...
function assertSourceUnchanged(paths) {
    let current;
    try {
        current = identity(fs.statSync(paths.supplied));
    }
    catch {
    ...
    if (!sameIdentity(paths.sourceIdentity, current))
        throw new DraftError('source_changed', 'input identity, size, or modification time changed during extraction', 'wait until the source file is stable, then run again', EXIT.FAILED);
}
function parseListing(text) {
    return new Set(text.split(/\r?\n/).flatMap(line => line.trim().split(/\s+/)));
}
async function platformPreflight(manager) {
    if (!versionAtLeast(process.version, MINIMUM_NODE))
        throw new DraftError('node_version_unsupported', `Node.js 20.6.0 or newer is required, running ${process.version}`, 'install Node.js 20.6.0 or newer');
    if (process.platform !== 'darwin')
        throw new DraftError('platform_unsupported', `unsupported platform: ${process.platform}`, 'run this skill on macOS 26.0 or newer');
    const swVers = resolveCommand('sw_vers');
    if (!swVers)
        throw new DraftError('command_missing', 'macOS sw_vers was not found', 'restore /usr/bin/sw_vers, which ships with macOS');
    const result = await manager.run(swVers, ['-productVersion']);
    const version = result.stdout.trim();
    if (result.code !== 0 || !versionAtLeast(version, MINIMUM_MACOS))
        throw new DraftError('platform_unsupported', `macOS 26.0 or newer is required, running ${version || 'an unknown version'}`, 'upgrade this Mac to macOS 26.0 or newer');
...
async function toolchainPreflight(manager, platform) {
    ...
    let publisher = null;
    try {
        fs.accessSync('/usr/bin/osascript', fs.constants.X_OK);
        publisher = '/usr/bin/osascript';
    }
    catch { }
    ...
    for (const name of ['ffmpeg', 'ffprobe'])
        if (!commands[name])
            failures.push({ code: 'command_missing', condition: `required command not found: ${name}`, remedy: commandRemedy() });
    if (!commands.publisher)
        failures.push({ code: 'publication_unsupported', condition: 'required publication command not found: /usr/bin/osascript', remedy: 'restore /usr/bin/osascript, which ships with macOS' });
    if (!commands.swiftc)
        failures.push({ code: 'command_missing', condition: 'required command not found: swiftc', remedy: 'install the macOS Command Line Tools with xcode-select --install' });
    if (!commands.sips)
        failures.push({ code: 'command_missing', condition: 'required command not found: sips', remedy: 'restore /usr/bin/sips, which ships with macOS' });
    if (commands.ffmpeg) {
        for (const [flag, wanted] of [
            ...
            if (result.code !== 0)
                failures.push({ code: 'ffmpeg_probe_failed', condition: `ffmpeg could not report ${flag.slice(1)}`, remedy: commandRemedy() });
            else {
                const listing = parseListing(result.stdout + result.stderr);
                for (const capability of wanted)
                    if (!listing.has(capability))
                        failures.push({ code: 'ffmpeg_capability_missing', condition: `ffmpeg is missing required capability: ${capability}`, remedy: commandRemedy() });
            }
        }
    }
    if (commands.ffprobe) {
        ...
        if (result.code !== 0 || !result.stdout.trim())
            failures.push({ code: 'ffprobe_probe_failed', condition: 'ffprobe could not report its version', remedy: commandRemedy() });
    }
    if (failures.length || !commands.ffmpeg || !commands.ffprobe || !commands.publisher || !commands.swiftc || !commands.sips)
        throw new DraftError('preflight_failed', `${failures.length} toolchain preflight check(s) failed`, commandRemedy(), EXIT.CANNOT_START, { failures });
...
async function readJson(manager, command, args, code, condition, remedy, exitCode = EXIT.CANNOT_START) {
    ...
    if (mediaFailed(result))
        throw new DraftError(code, `${condition}${result.stderr.trim() ? `: ${result.stderr.trim()}` : ''}`, remedy, exitCode, childDetails(code, result));
    try {
        return JSON.parse(result.stdout);
    }
    catch {
        throw new DraftError(code, `${condition}: output was not valid JSON`, remedy, exitCode, childDetails(code, result));
    }
}
async function inspectInput(manager, state, options) {
    ...
    if (typeof stream.width !== 'number' || typeof stream.height !== 'number' || !stream.width || !stream.height)
        throw new DraftError('stream_unsupported', 'selected video stream has no valid dimensions', 're-export the source with a decodable video stream');
    ...
    if (mediaFailed(result)) {
        ...
        if (/no space left on device|ENOSPC|disk quota exceeded/i.test(result.stderr))
            throw spoolStorageError(spool, new Error(result.stderr.trim()), details);
    ...
    try {
        ...
        if (error instanceof DraftError && error.code === 'input_unusable' && error.condition.endsWith('output was not valid JSON'))
            Object.assign(error, childDetails('input_unusable', result));
        throw error;
    }
...
async function representativeDecodePreflight(manager, state) {
    ...
    if (mediaFailed(result))
        throw new DraftError('input_decode_failed', `ffmpeg could not decode and convert a representative selected frame${result.stderr.trim() ? `: ${result.stderr.trim()}` : ''}`, 'repair or re-export the input with a supported video codec and color description', EXIT.CANNOT_START, childDetails('input_decode_failed', result));
    if (!/(?:^|\n)frame=1(?:\r?\n|$)/.test(result.stdout))
        throw new DraftError('input_decode_failed', 'ffmpeg completed the representative decode probe without producing the selected frame', 'repair or re-export the input with valid presentation timestamps', EXIT.CANNOT_START, childDetails('input_decode_failed', result));
    if (hdr) {
...
async function syntheticEncoderPreflight(manager, state) {
    ...
    if (mediaFailed(result))
        throw new DraftError('heic_encoder_unavailable', `ffmpeg could not create the synthetic HLG preflight frame${result.stderr.trim() ? `: ${result.stderr.trim()}` : ''}`, 'repair ffmpeg-full and run the same command again', EXIT.CANNOT_START, childDetails('heic_encoder_unavailable', result));
...
async function encodeHeic(manager, state, input, output, exitCode = EXIT.FAILED) {
    ...
    if (!encoder)
        throw new DraftError('heic_encoder_unavailable', 'Swift HEIC helper has not been compiled', 'run preflight before encoding HDR frames', exitCode);
    const result = await manager.run(encoder, [input, output, hdrTransfer(state.media.color)]);
    if (result.code !== 0)
        throw new DraftError('heic_encode_failed', `HDR TIFF-to-HEIC encoding failed${result.stderr.trim() ? `: ${result.stderr.trim()}` : ''}`, 'repair the reported Core Image encoder failure and run again', exitCode, childDetails('heic_encode_failed', result));
}
async function inspectHeic(manager, state, filename, displayName, exitCode = EXIT.FAILED) {
    ...
    if (result.code !== 0)
        throw new DraftError('structural_check_failed', `sips could not inspect extracted frame: ${displayName}${result.stderr.trim() ? `: ${result.stderr.trim()}` : ''}`, 'repair the Core Image HEIC encoder and run again', exitCode, childDetails('structural_check_failed', result));
    ...
    if (width !== state.media.width || height !== state.media.height || depth !== 10 || !wantedProfile.test(profile))
        throw new DraftError('structural_check_failed', `frame structure does not match 10-bit ${state.media.color.outputColor} HEIC ${state.media.width}x${state.media.height}: ${displayName}`, 'repair the FFmpeg TIFF conversion or Core Image HEIC encoder and run again', exitCode);
...
async function convertHdrFrames(manager, state, temporary) {
    ...
    if (files.length !== state.media.expectedFrames)
        throw new DraftError('structural_check_failed', `expected ${state.media.expectedFrames} TIFF intermediates but found ${files.length}`, 'repair the reported FFmpeg extraction failure and run again', EXIT.FAILED);
    let next = 0;
    let failure = null;
...
async function structuralChecks(manager, state, temporary) {
    ...
    if (files.length !== state.media.expectedFrames)
        throw new DraftError('structural_check_failed', `expected ${state.media.expectedFrames} frames but found ${files.length}`, 'repair the reported FFmpeg extraction failure and run again', EXIT.FAILED);
    for (const file of files)
        if (fs.statSync(path.join(temporary, file)).size <= 0)
            throw new DraftError('structural_check_failed', `empty frame file: ${file}`, 'repair the reported FFmpeg encoder failure and run again', EXIT.FAILED);
    const probes = [];
    for (const file of [files[0], files.at(-1)]) {
        ...
        if (!stream || stream.codec_name !== state.media.color.codec || stream.width !== state.media.width || stream.height !== state.media.height)
            throw new DraftError('structural_check_failed', `frame structure does not match ${state.media.color.codec} ${state.media.width}x${state.media.height}: ${file}`, 'repair the FFmpeg filter or image encoder and run again', EXIT.FAILED);
        let properties;
        try {
            properties = pixelProperties(stream, state.pixelDescriptors);
        }
        catch (error) {
        ...
        if (properties.alpha !== state.media.color.alpha || properties.bitDepth !== Number(state.media.color.outputDepth))
            throw new DraftError('structural_check_failed', `frame alpha/depth does not match alpha=${state.media.color.alpha}, depth=${state.media.color.outputDepth}: ${file} (${stream.pix_fmt})`, 'repair the FFmpeg filter or PNG encoder and run again', EXIT.FAILED);
...
function emitCleanupFailures(failures) {
    if (Array.isArray(failures) && failures.length) {
        ...
        for (const raw of failures) {
            const failure = errorFields(raw);
            process.stderr.write(`  [${failure.code}] ${failure.path}: ${failure.condition}\n`);
        }
    }
}
function cleanupPaths(paths) {
    const failures = [];
    for (const pathname of paths.filter((value) => Boolean(value))) {
        try {
            fs.rmSync(pathname, { recursive: true, force: true });
        }
        catch (error) {
            failures.push({ path: path.resolve(pathname), code: errorFields(error).code || 'cleanup_failed', condition: errorText(error) });
        }
    }
    return failures;
}
function emitError(error, json) {
    ...
    const payload = { code: fields.code || 'unexpected_failure', condition: fields.condition || fields.message, remedy: fields.remedy || 'inspect the reported failure and run again' };
    ...
    if (json)
        ...
        if (Array.isArray(payload.failures))
            for (const failure of payload.failures.map(errorFields))
                process.stderr.write(`  [${failure.code}] ${failure.condition}\n      Remedy: ${failure.remedy}\n`);
        process.stderr.write(`Remedy: ${payload.remedy}\n`);
        emitCleanupFailures(payload.cleanupFailures);
        for (const key of ['task', 'childExitCode', 'childSignal', 'stderr'])
...
async function compileEncoder(manager, state, encoderDirectory) {
    ...
    if (result.code !== 0)
        throw new DraftError('heic_encoder_unavailable', `Swift HEIC helper could not compile${result.stderr.trim() ? `: ${result.stderr.trim()}` : ''}`, 'install or repair the macOS Command Line Tools with xcode-select --install', EXIT.CANNOT_START, childDetails('heic_encoder_unavailable', result));
...
async function prepare(manager, options, encoderDirectory) {
    try {
        ...
        if (paths) {
            ...
            if (prepared.media.color.dynamicRange !== 'sdr')
                await compileEncoder(manager, prepared, encoderDirectory);
...
async function publishDirectoryNoReplace(manager, state, temporary) {
    ...
    let result;
    try {
        result = await manager.run(state.commands.publisher, args);
    }
    catch (error) {
    ...
    if (result.code === 0 && result.stdout.trim() === 'published')
        return;
...
async function main(argv) {
    ...
    try {
        ...
        if (options.help) {
            process.stdout.write(usage(path.basename(process.argv[1] || 'extract-video-frames.js')));
            return EXIT.OK;
        }
        ...
        if (options.preflight) {
            ...
            if (!state.paths || !state.media)
                throw new DraftError('input_unusable', 'input preparation did not produce media', 'pass a readable video input');
            if (pathExists(state.paths.output))
                throw new DraftError('output_collision', `output appeared during preflight: ${state.paths.output}`, 'move or remove the existing path, then run again');
            ...
            manager.assertRunning();
            if (mediaFailed(extraction))
                throw new DraftError('extraction_failed', `ffmpeg extraction failed${extraction.stderr.trim() ? `: ${extraction.stderr.trim()}` : ''}`, 'fix the reported decode, color-conversion, or image-encoder error and run again', EXIT.FAILED, childDetails('extraction', extraction));
            if (state.media.color.dynamicRange !== 'sdr')
                await convertHdrFrames(manager, state, temporary);
            manager.assertRunning();
        ...
        if (manager.active.size || manager.signal)
            await manager.interrupt(manager.signal || 'SIGTERM');
    ...
    if (manager.signal) {
        if (cleanupFailures.length)
            emitError(new DraftError('interrupted', `interrupted by ${manager.signal}`, 'inspect the retained paths before deciding whether to remove them or run again', SIGNAL_EXIT[manager.signal], { cleanupFailures }), json);
        return SIGNAL_EXIT[manager.signal];
    }
    if (primaryError) {
        if (cleanupFailures.length)
            Object.assign(primaryError, { cleanupFailures });
    ...
    if (!payload)
        throw new Error('extraction completed without a report');
    if (cleanupFailures.length)
...
if (require.main === module)
    main(process.argv.slice(2)).then(code => { process.exitCode = code; }, error => { emitError(error, process.argv.includes('--json')); process.exitCode = EXIT.FAILED; });
