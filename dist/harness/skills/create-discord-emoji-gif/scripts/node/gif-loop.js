...
exports.inspectGifLoop = inspectGifLoop;
...
function inspectGifLoop(buffer) {
    ...
    const colorTable = (packed) => { if (packed & 0x80)
        take(3 * (2 ** ((packed & 7) + 1))); };
    ...
    while (offset < buffer.length) {
        ...
        if (sentinel === 0x3b) {
            if (offset !== buffer.length)
                fail('data after trailer');
            if (!loop)
                fail('missing supported loop declaration');
            if (loop.repeatCount !== 0)
                fail(`finite repetition count ${loop.repeatCount}`);
        ...
        if (sentinel === 0x2c) {
            ...
            if (byte() !== 11)
                fail('application identifier block must contain 11 bytes');
            const extension = take(11).toString('latin1');
            if (extension !== 'NETSCAPE2.0' && extension !== 'ANIMEXTS1.0') {
                subBlocks();
                continue;
            }
            if (byte() !== 3)
                fail('loop control payload must contain three bytes');
            const control = take(3);
            if (control[0] !== 1)
                fail('unsupported loop control subcode');
            const repeatCount = control.readUInt16LE(1);
            if (byte() !== 0)
                fail('ambiguous loop control payload or missing terminator');
            if (loop && loop.repeatCount !== repeatCount)
                fail('conflicting loop declarations');
