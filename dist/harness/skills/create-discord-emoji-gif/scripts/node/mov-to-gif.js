...
exports.candidateTasks = candidateTasks;
...
function candidateTasks(config) {
    ...
    for (let fps = config.minFps; fps <= config.maxFps; fps += 1)
        for (let colors = 4; colors <= 256; colors += 1)
            tasks.push({ fps, colors });
    return tasks;
}
...
async function checked(state, task, command, args, options, code, condition, remedy) {
    ...
    if (result.code !== 0 || result.signal || (command === state.commands.ffmpeg && shared.mediaFailed(result)))
        throw shared.subprocessError(code, `${condition}${result.stderr.trim() ? `: ${result.stderr.trim()}` : ''}`, remedy, task, result);
    return result;
}
...
async function convert(initial) {
    if (!initial.json)
        process.stderr.write(`Searching ${initial.config.minFps}-${initial.config.maxFps} FPS, 4-256 colors, dithers 2-5 under ${initial.config.maxBytes} bytes at ${initial.config.gifSize}x${initial.config.gifSize} with ${initial.config.jobs} workers...\n`);
    ...
    if (!winner)
        throw new shared.RunError('no_candidate', `no candidate fit below ${state.config.maxBytes} bytes`, 'increase MAX_BYTES, reduce GIF_SIZE, or reduce the FPS range');
