...
[Development](README.md) / Container

Commands below run from the repository root. Ordinary macOS development runs natively. Use this guide on Linux/WSL2 or for explicit Docker validation on macOS.
...
The [dependency inventory](dependencies.md#linux-host-and-container) lists the image toolchain. The [Dockerfile](../../Dockerfile) implements it.
...
See [platform validation records](validation.md) for dated container and native macOS results and coverage gaps.
